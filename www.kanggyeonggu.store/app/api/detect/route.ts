...
/**
 * FastAPI 서버로 디텍션 요청을 전달하는 프록시 API
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // FastAPI 서버에 디텍션 요청 전달
      const response = await fetch(`${fastApiUrl}/api/detect`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        return NextResponse.json(
          {
            error: data.detail || '디텍션 중 오류가 발생했습니다.',
            details: data.detail,
          },
      ...
      return NextResponse.json(
        { error: '디텍션 중 오류가 발생했습니다.', details: error.message },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error('디텍션 API 오류:', error);
    return NextResponse.json(
      { error: '디텍션 요청 처리 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
