...
/**
 * FastAPI 서버로 세그먼테이션 요청을 전달하는 프록시 API
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // FastAPI 서버에 세그먼테이션 요청 전달
      const response = await fetch(`${fastApiUrl}/api/segment`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        return NextResponse.json(
          {
            error: data.detail || '세그먼테이션 중 오류가 발생했습니다.',
            details: data.detail,
          },
      ...
      return NextResponse.json(
        { error: '세그먼테이션 중 오류가 발생했습니다.', details: error.message },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error('세그먼테이션 API 오류:', error);
    return NextResponse.json(
      { error: '세그먼테이션 요청 처리 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
