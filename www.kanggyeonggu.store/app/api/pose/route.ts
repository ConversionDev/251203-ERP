...
/**
 * FastAPI 서버로 포즈 추정 요청을 전달하는 프록시 API
...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            // FastAPI 서버에 포즈 추정 요청 전달
            const response = await fetch(`${fastApiUrl}/api/pose`, {
                method: 'POST',
                headers: {
            ...
            return NextResponse.json(
                { error: '포즈 추정 중 오류가 발생했습니다.', details: error.message },
                { status: 500 }
            );
        }
    } catch (error) {
        ...
        return NextResponse.json(
            { error: '포즈 추정 요청 처리 중 오류가 발생했습니다.' },
            { status: 500 }
        );
    }
}
