package store.kanggyoenggu.oauthservice.google;
...
import store.kanggyoenggu.oauthservice.response.*;
...
public class GoogleController {
    ...
    private final store.kanggyoenggu.oauthservice.service.UserService userService;
    ...
    public GoogleController(GoogleOAuthService googleOAuthService, JwtService jwtService, store.kanggyoenggu.oauthservice.service.UserService userService) {
    ...
    private ResponseEntity<Void> processCallback(String code) {
        try {
            ...
            String profileImageUrl = userInfo.getPicture();

            // 4. DB Upsert + Redis 저장 (핵심!)
            store.kanggyoenggu.oauthservice.entity.User user = userService.upsertUser(
                    "google",
                    googleId,
                    nickname,
                    profileImageUrl
